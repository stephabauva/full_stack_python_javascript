    ...
    def post(self, request, format=None):
        ...
        if code != None:
            ...
            if len(room_result) > 0:
                room = room_result[0]
                #add room_code info to the session data so that if the user leaves, it won't
                #need to re-enter the room code of the last session when he comes back
...
class UserInRoom(APIView):
    def get(self, request, format=None):
        ...
        #put roomCode in a dictionary
        data = {
            'code': self.request.session.get('room_code')
        }
        #serialize and return the python dictionary
...
class SessionInfo(APIView):
    def get(self, request, format=None):
        ...
        session_info['session_key'] = self.request.session.session_key
        return JsonResponse(session_info, status=status.HTTP_200_OK)
